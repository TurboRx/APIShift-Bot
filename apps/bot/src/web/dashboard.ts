...
<body>
    ...
    <div id="tab-workbench" class="tab-content active">
      <div class="card-panel">
        ...
        <div class="metrics-bar">
          ...
          <div>Latency: <span class="metric-pill" id="metrics-latency">0.0 ms</span></div>
        </div>
      </div>
    </div>
  ...
  <script>
    (function () {
      ...
      var PRESETS = ${JSON.stringify(PRESETS_DATA)};
      ...
      async function runMonacoRefactor() {
        ...
        try {
          ...
          const data = await safeFetchJson('/api/refactor', {
            method: 'POST',
      ...
      async function runSchemaDiff() {
        ...
        try {
          ...
          const result = await safeFetchJson('/api/diff', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
      ...
      async function checkSpecWatcher() {
        ...
        try {
          const data = await safeFetchJson('/api/cron/spec-watcher');
      ...
      async function dispatchFleetMigration(e) {
        ...
        try {
          const data = await safeFetchJson('/api/fleet/migrate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
      ...
      async function fetchQueueStatus() {
        try {
          const data = await safeFetchJson('/api/queue/status');
          if (data.stats) {
      ...
      async function enqueueTestWebhook() {
        try {
          await safeFetchJson('/api/queue/enqueue', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
